...
static PyObject *Matrix61c_add(Matrix61c* self, PyObject* args) {
	...
	if (!PyObject_TypeCheck(args, &Matrix61cType)) {
	    ...
	    int rows = self->mat->rows;
	    int cols = self->mat->cols;
	   //Error Checking: IndexError, RuntimeError.
	    int mat_check = allocate_matrix(&result_mat, rows, cols); 	
	    if(mat_check == -1) {
	    	 PyErr_SetString(PyExc_IndexError, "Index out of range");
		 return NULL;
	    }
	    if (mat_check == -2) {
		 ...
		 PyErr_SetString(PyExc_RuntimeError, "Failed to allocate space");
		 return NULL; 
	    }
	
    	    		    
	    //Initializing Matrix61c* result. Assigning result_mat to result.
	    if (add_matrix(result_mat, self->mat, mat61c->mat) != 0) {
	    	deallocate_matrix(result_mat);
		    PyErr_SetString(PyExc_TypeError, "Dimensions do not match!");
		    return NULL;
	    } 
	    Matrix61c* result61c = (Matrix61c*) Matrix61c_new(&Matrix61cType, NULL, NULL);
	    result61c->mat = result_mat;
	    result61c->shape = PyTuple_Pack(2, PyLong_FromLong(rows), PyLong_FromLong(cols));
	    return result61c;
...
static PyObject *Matrix61c_sub(Matrix61c* self, PyObject* args) {
	...
	if (!PyObject_TypeCheck(args, &Matrix61cType)) {
	   ...
	   //Error Checking: IndexError, RuntimeError.
	    ...
	    //Initializing Matrix61c* result. Assigning result_mat to result.
	    if (sub_matrix(result_mat, self->mat, mat61c->mat) != 0) {
	    	deallocate_matrix(result_mat);
		    PyErr_SetString(PyExc_TypeError, "Dimensions do not match!");
		    return NULL;
	    } 
...
static PyObject *Matrix61c_multiply(Matrix61c* self, PyObject *args) {
	...
	if (!PyObject_TypeCheck(args, &Matrix61cType)) {
	   ...
	   //Error Checking: IndexError, RuntimeError.
	    ...
	    //Initializing Matrix61c* result. Assigning result_mat to result.
	    if (mul_matrix(result_mat, self->mat, mat61c->mat) != 0) {
	    	deallocate_matrix(result_mat);
		    PyErr_SetString(PyExc_TypeError, "Dimensions do not match!");
		    return NULL;
	    } 
...
static PyObject *Matrix61c_pow(Matrix61c *self, PyObject *pow, PyObject *optional) {
    /* TODO: YOUR CODE HERE */
	if (!PyLong_Check(pow)) {
	    PyErr_SetString(PyExc_TypeError, "Argument must of type Pylong!");
	    return NULL;
	} else {   		
	   ...
	   //Error Checking: IndexError, RuntimeError.
	    ...
	    //Initializing Matrix61c* result. Assigning result_mat to result.
	    if (pow_matrix(result_mat, self->mat, pow_C) != 0) {
	    	deallocate_matrix(result_mat);
		    PyErr_SetString(PyExc_TypeError, "Dimensions do not match!");
		    return NULL;
	    } 
...
static PyObject *Matrix61c_set_value(Matrix61c *self, PyObject* args) {
    ...
    if (PyArg_UnpackTuple(args, "args", 3, 3, &rows, &cols, &val)) {
        if (rows && cols && val && PyLong_Check(rows) && PyLong_Check(cols) && PyFloat_Check(val)) {
            int C_long_cols = PyLong_AsLong(cols);
            int C_long_rows = PyLong_AsLong(rows);
            if ((C_long_cols >= 0) && (C_long_rows >= 0)) {
                set(self->mat, C_long_rows, C_long_cols, PyFloat_AsDouble(val));
                return NULL;
            } else {
...
static PyObject *Matrix61c_get_value(Matrix61c *self, PyObject* args) {
    ...
    if (PyArg_UnpackTuple(args, "args", 2, 2, &rows, &cols)) {
        if (rows && cols && PyLong_Check(rows) && PyLong_Check(cols)) {
            ...
            int C_long_rows = PyLong_AsLong(rows);
            if ((C_long_cols >= 0) && (C_long_rows >= 0)) {
                get(self->mat, C_long_rows, C_long_cols);
                return NULL;
            } else {
                PyErr_SetString(PyExc_IndexError, "Invalid arguments");
                return NULL   
...
static PyMethodDef Matrix61c_methods[] = {
    /* TODO: YOUR CODE HERE */
    {"get", Matrix61c_get_value, METH_VARARGS, "get function from C"},
    {"set", Matrix61c_set_value, METH_VARARGS, "set function from C"},
    {NULL, NULL, 0, NULL}
};
